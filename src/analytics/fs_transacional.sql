...
tb_agg_transacao AS (
    ...
    SELECT 
        ...
        SUM(DISTINCT CASE WHEN dtDia >= date('{date}', '-7 day') THEN qtdePontos else 0 END) AS saldoD7,
        SUM(DISTINCT CASE WHEN dtDia >= date('{date}', '-14 day') THEN qtdePontos else 0 END) AS saldoD14,
        SUM(DISTINCT CASE WHEN dtDia >= date('{date}', '-28 day') THEN qtdePontos else 0 END) AS saldoD28,
        SUM(DISTINCT CASE WHEN dtDia >= date('{date}', '-56 day') THEN qtdePontos else 0 END) AS saldoD56,
        ...
        SUM(DISTINCT CASE WHEN dtDia >= date('{date}', '-7 day') AND qtdePontos < 0 THEN qtdePontos else 0 END) AS qtdePontosNegD7,
        ...
        SUM(DISTINCT CASE WHEN dtDia >= date('{date}', '-56 day') AND qtdePontos < 0 THEN qtdePontos else 0 END) AS qtdePontosNegD56,
...
tb_share_produtos AS (
    ...
    SELECT 
        ...
        1. * COUNT(CASE WHEN DescNomeProduto = 'Troca de Pontos StreamElement' THEN t1.IdTransacao END) / COUNT(t1.IdTransacao) AS qteTrocaStreamElements,
        1. * COUNT(CASE WHEN DescNomeProduto = 'Reembolso: Troca de Pontos StreamElement' THEN t1.IdTransacao END) / COUNT(t1.IdTransacao) AS qteReembolsoStreamElements,
